...
def train(field, train_data):
	...
	with open("%s/pretrain_%s_predict.pkl" % (mention_valid_file, field), "rb") as f:
	...
	ckpt_file = model_file + "/" + "lmpretrain_em_predict_pretrain_%s_16.ckpt" % field
	...
	for epoch in range(epochs // 2):
		...
		print("epoch {} train_instance_num: ".format(epoch) ,  epoch_train_instance_num)

		model.train()
		total_loss = 0.0
		for i in range((epoch_train_instance_num-1) // batch_instance_num + 1):

			train_batch_token_ids, train_batch_token_masks, \
			train_batch_segment_ids, train_batch_labels = get_batch_data(field_predict_data, i, batch_instance_num)
	...
	for epoch in range(epochs):
		...
		# mode_save_file = model_file + "/train_el_{}.ckpt".format(epoch+1)
